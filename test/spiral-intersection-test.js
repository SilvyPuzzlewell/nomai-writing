...
// Check spiral intersection with proximity detection
// Uses sampled Bezier curves and checks both crossing AND minimum distance
// to account for line width (spirals are drawn up to 3.5px wide)
function checkSpiralIntersection(newPoints, existingSpirals, skipSegments = 3) {
    ...
    const minDistance = 4.0; // Matches production tolerance for line width
    ...
    for (const existing of existingSpirals) {
        ...
        for (let i = skipNewSegments; i < sampledNew.length - 1; i++) {
            for (let j = 0; j < sampledExisting.length - 1; j++) {
                ...
                // Check for proximity (points too close - would overlap when drawn)
                const dx = sampledNew[i].x - sampledExisting[j].x;
                const dy = sampledNew[i].y - sampledExisting[j].y;
...
function checkPixelCoincidence(layoutNodes, samplesPerSegment = 20, skipPoints = 5) {
    ...
    spirals.forEach((spiral, spiralIndex) => {
        ...
        sampledPoints.slice(skipPoints).forEach(point => {
...
for (let testNum = 0; testNum < numRandomTests; testNum++) {
    ...
    if (passed) {
        ...
        randomTestsFailed++;
        // Only count as failed test if collision rate is high (>10% of nodes)
        if (collisionCount > messages.length * 0.1) {
            failedTests++;
        }
...
console.log(`Collision rate: ${(totalCollisions / totalNodes * 100).toFixed(1)}% (${totalCollisions} collisions in ${totalNodes} nodes)`);

// Save all test layouts to JSON
const fs = require('fs');
const outputPath = 'test/test-layouts.json';
...
if (randomTestsFailed > 0) {
    const failedPath = 'test/failed-layouts.json';
...
console.log("\n" + "=".repeat(50));
console.log(`FINAL: ${testCases.length + randomTestsPassed - failedTests}/${testCases.length + numRandomTests} total passed`);
console.log("=".repeat(50));
